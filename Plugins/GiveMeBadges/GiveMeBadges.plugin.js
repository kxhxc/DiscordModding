	...
	(() => {
		...
		(() => {
			...
			const boosts = [{
				...
				id: "boost6",
				value: 0 << 0,
				name: "Server Booster - 1 Year",
				time: 12.04
			}, {
				id: "boost7",
				value: 0 << 0,
				name: "Server Booster - 1 Year and 3 Months",
				time: 15.04
			}, {
				id: "boost8",
				value: 0 << 0,
				name: "Server Booster - 1 Year and 6 Months",
				time: 18.04
			}, {
				id: "boost9",
				value: 0 << 0,
				name: "Server Booster - 2 Years",
				time: 24.04
			}];
			...
			const flush = new Set;
			class GiveMeBadges extends(external_BasePlugin_default()) {
				constructor(...args) {
					super(...args);
				...
				async patchUserContextMenus() {
					const getMenu = props => {
						...
						return React.createElement(contextmenu_namespaceObject.MenuItem, {
							...
							children: [modules_UserFlags.map((flag => {
								...
								label: "Reset Server Boost Badges",
								id: "reset-boosts",
								color: "colorDanger",
								action: () => {
									delete settings[props.user.id]?.boost;
									this.saveSettings(settings);
									external_PluginApi_namespaceObject.Toasts.success(`Successfully cleared boost badges for <strong>${props.user}</strong>!`);
								}
							}))]), React.createElement(contextmenu_namespaceObject.MenuGroup, null, React.createElement(contextmenu_namespaceObject.MenuItem, {
								color: "colorDanger",
								label: "Reset Badges",
								...
								action: () => {
									...
									external_PluginApi_namespaceObject.Toasts.success(`Successfully cleared badges for <strong>${props.user}</strong>!`);
								}
							}))]
						});
					};
					...
					const originalSymbol = Symbol("GiveMeBadges Original");
					const search = async () => {
...
module.exports = window.hasOwnProperty("ZeresPluginLibrary") ?
	...
	class {
		...
		getDescription() {
			return `${config.info.description}. __**If you are using BD (BetterDiscord), You need ZeresPluginLibrary, or this plugin will not work.**__`;
		}
		getVersion() {
		...
		load() {
			BdApi.showConfirmationModal(
				...
				[`Plugin Library for ${config.info.name} is missing. Please click Download to install it .`], {
